package com.smubator.adwitter.Carousel;
...
public class CoverFlowCarousel extends Carousel {
    ...
    protected View addAndMeasureChild(final View child, final int layoutMode) {
        if (child.getLayoutParams() == null) child.setLayoutParams(new ViewGroup.LayoutParams(mChildWidth,
            mChildHeight));

        final int index = layoutMode == LAYOUT_MODE_TO_BEFORE ? 0 : -1;
        addViewInLayout(child, index, child.getLayoutParams(), true);

        final int pwms = View.MeasureSpec.makeMeasureSpec(mChildWidth, View.MeasureSpec.EXACTLY);
        final int phms = View.MeasureSpec.makeMeasureSpec(mChildHeight, View.MeasureSpec.EXACTLY);
